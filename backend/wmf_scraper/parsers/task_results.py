...
def get_competition_id_from_name(competitor_name: str, competitors_mapping: dict | None) -> int:
    if competitors_mapping and competitor_name in competitors_mapping:
        return competitors_mapping[competitor_name]
    return -1


def get_task_results(task_data: TaskModel, competitors_mapping: dict | None = None) -> list[TaskResultModel]:
    ...
    for task_results_info in page.findall(".//tbody"):
        ...
        for task_result in task_results:
            competitor_name = task_result.findall(".//span[@class='fw-semibold']")[0].text_content().split(" - ")[1]
            result_content = [td.text_content() for td in task_result.findall(".//td")][2:]
            result.append(
                TaskResultModel(
                    task_id=task_data.task_id,
                    competitor_id=get_competition_id_from_name(competitor_name, competitors_mapping),
...
def get_task_results_parallel(tasks: list[TaskModel], competitors: dict) -> list[TaskResultModel]:
    result = []
